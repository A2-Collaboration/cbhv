...
void Karte(UInt_t start, UInt_t stop, Bool_t SaveTxt = true, char* TxtFileName = "HV_gains_offsets.txt", Bool_t SavePs = true){	
  ...
  char Slevel[1000];
  Float_t chn[8][1000];
  char Schn0[1000], Schn1[1000], Schn2[1000], Schn3[1000], Schn4[1000], Schn5[1000], Schn6[1000], Schn7[1000];
  TH1F *HistHV[1000][8];
  char HistName[1000];
  char HistTitle[1000];
  ...
  for(UInt_t i=start; i<=stop; i++){
    ...
    if(InFile){
      ...
      fscanf(InFile, "%[^,],%*s %*[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%s", SSetHV, Slevel, Schn0, Schn1, Schn2, Schn3, Schn4, Schn5, Schn6, Schn7); // Read in normal line, use comma as delimiter (mostly)
	...
	if(SaveTxt) fprintf (TxtFile, "%d,%d,%f,%f\r\n", i, j, par1, par0);
      }
      hvcanv->Update();
      if(SavePs){
	ps->Close();
	printf("Saved histogrammes to %s\n", PsFileName);
      }
      fclose(InFile);
    }
    else{
